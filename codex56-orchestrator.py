...
主要职责：
  1. 自动识别当前账户使用的 Codex 配置位置，也可通过 --codex-dir 指定目标
  2. 把内置规则或用户选择的 Markdown 文档放入 .codex 目录
  3. 将 config.toml 的 model_instructions_file 指向目标规则文件
  4. 发现可能影响规则加载的 hooks.json 时进行检测和隔离

执行约定：
  - 未提供 --yes 时仅展示预计变更，不落盘
  - --name 接受普通文件名，不接受绝对路径或跨目录写法
  - 改动配置与同名规则文件前会创建备份
  - hooks.json 隔离前会生成带时间标记的副本，之后可用 --restore-hooks 还原

常用命令：
  python3 codex56-orchestrator.py --dry-run
  python3 codex56-orchestrator.py --codex-dir ~/.codex --yes
  python3 codex56-orchestrator.py --codex-dir ~/.codex --restore-hooks
  python3 codex56-orchestrator.py --file my_rules.md --name my-rules --codex-dir ~/.codex --yes
"""
...
def _deploy_locked(args, codex_dirs: Optional[List[str]] = None) -> None:
    ...
    if not codex_dirs:
        ...
        _print(
            _localized(
                "    手动指定: python3 codex56-orchestrator.py --codex-dir ~/.codex --dry-run",
                "    Specify explicitly: python3 codex56-orchestrator.py --codex-dir ~/.codex --dry-run",
            )
        )
        sys.exit(1)
  ...
  %(prog)s --file ./my_prompt.md --dry-run   Use an external Markdown file
    ...
    if args.restore_hooks:
        ...
        if not codex_dirs:
            ...
            _print(
                _localized(
                    "    手动指定: python3 codex56-orchestrator.py --codex-dir ~/.codex --restore-hooks",
                    "    Specify explicitly: python3 codex56-orchestrator.py --codex-dir ~/.codex --restore-hooks",
                )
            )
            sys.exit(1)
