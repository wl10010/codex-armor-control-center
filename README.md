...
- **一键部署**：部署内置规则，并更新 Codex 的 `model_instructions_file`。
- **一键卸载**：依据部署清单恢复配置和相关文件。
- **状态识别**：区分未部署、已部署、配置冲突、事务待恢复和部署受阻。
- **重新部署**：部署后始终保留入口；托管内容异常时自动切换为修复入口。
...
`config.toml` 中的模型、MCP、插件、Grok socket 等其他字段不属于本工具的托管范围，因此这些字段发生变化时，界面仍会显示“已部署”。必要时程序会同步部署清单，并保留 `.codex-keysmith-manifest.json.auto_sync.bak` 滚动备份。
